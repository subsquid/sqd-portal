...
async fn run_smoke(ctx: &Ctx, portal_proc: &mut PortalProcess) -> anyhow::Result<()> {
    ...
    let catalog_text = String::from_utf8_lossy(&catalog.body);
        ...
        catalog_text.contains("toy"),
        "catalog misses toy: {catalog_text}"
    );
    ensure!(
        catalog_text.contains("toy-rt"),
        "catalog misses toy-rt: {catalog_text}"
    );
    ...
    let head_json: serde_json::Value = serde_json::from_slice(&head.body)?;
        ...
        head_json["number"].as_u64() == Some(99),
        "archival head number: {head_json}"
    );
    ensure!(
    ...
    .await?;
    check_clean(
        "archival single-chunk stream",
        &validate_stream(world, &req, &expect, &resp),
        &resp,
    )?;
    ...
    let expect = model(world, &req);
    let sel = driver::stream(
        http,
        base,
        "toy",
        "finalized-stream",
        &selective_query(10, 50, None),
        "ct1-sel-net",
    )
    .await?;
    check_clean(
        "selective network multi-chunk tail",
        &validate_stream(world, &req, &expect, &sel),
        &sel,
    )?;
    ensure!(
    ...
    let expect = model(world, &req);
    let cont = driver::stream(
        http,
        base,
        "toy",
        "finalized-stream",
        &selective_query(51, 79, None),
        "ct1-sel-net-cont",
    )
    .await?;
    check_clean(
        "selective network continuation",
        &validate_stream(world, &req, &expect, &cont),
        &cont,
    )?;
    ...
    let expect = model(world, &req);
    let sel_rt = driver::stream(
        http,
        base,
        "toy-rt",
        "stream",
        &selective_query(12, 22, None),
        "ct1-sel-rt",
    )
    .await?;
    check_clean(
        "selective real-time tail",
        &validate_stream(world, &req, &expect, &sel_rt),
        &sel_rt,
    )?;
    ensure!(
        sel_rt.block_numbers() == vec![12, 22],
        "selective rt boundary: {:?}",
        sel_rt.block_numbers()
    );
    let req = StreamReq {
        dataset: "toy-rt".into(),
        from: 23,
    ...
    let expect = model(world, &req);
    let cont_rt = driver::stream(
        http,
        base,
        "toy-rt",
        "stream",
        &selective_query(23, 33, Some(&world.hash("toy-rt", 22))),
        "ct1-sel-rt-cont",
    )
    .await?;
    check_clean(
        "selective real-time continuation",
        &validate_stream(world, &req, &expect, &cont_rt),
        &cont_rt,
    )?;
    ...
    .await?;
        ...
        matches!(expect, Expect::Empty { .. }),
        "model should expect EMPTY"
    );
    check_clean(
        "beyond-frontier EMPTY",
        &validate_stream(world, &req, &expect, &resp),
        &resp,
    )?;
    ...
    for (attempt, n) in &per_attempt {
        ...
        !queries.is_empty(),
        "archival streams produced no worker queries"
    );
    ensure!(
