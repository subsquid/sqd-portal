...
impl StreamController {
    ...
    fn pop_response(&mut self) -> Poll<Option<Result<ResponseChunk, RequestError>>> {
        ...
        let (result, range) = match slot.state {
            ...
            RequestState::Partial(PartialResult { data, next_range }) => {
                ...
                let slot = match self.start_querying_chunk(&next_range, chunk_index) {
                    Ok(slot) => slot,
                    Err(e) => {
    ...
    fn start_querying_chunk(
        ...
        let request = match self.send_query(&range, index) {
            ...
            Err(SendQueryError::Backoff(until)) => {
                PendingRequests::paused(until, self.request.retries as u8)
            }
            Err(e) => return Err(e),
        };
        Ok(Slot {
            range,
            state: RequestState::Pending(request),
        })
    }
