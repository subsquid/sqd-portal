//! How the portal proves to the control plane which portal is asking (DC-8).
//!
//! Signed rather than a shared bearer token: a captured signature cannot be
//! replayed against a different credential, and the control plane stores a
//! public key rather than a secret.
...
impl RequestSigner {
    ...
    /// The raw 32-byte key, not the peer id: a peer id wraps it in protobuf
    /// inside a multihash, so verifying one would need a libp2p implementation.
    ...
    /// `timestamp_secs` is passed in, not read here, to keep the canonical form
...
/// Binding the body digest is what makes a captured signature useless against
/// another credential; the timestamp bounds how long it works against the same
/// one. No field may contain a newline, so no two requests share a form.
fn canonical(
    portal_id: &str,
    timestamp_secs: u64,
    method: &str,
...
mod tests {
    ...
    /// The control plane matches these by name. Both sides of every other test
    /// read the same constants, so only a literal here fails when one is renamed.
    #[test]
    fn the_header_names_are_pinned() {
        assert_eq!(PORTAL_ID_HEADER, "x-portal-id");
    ...
    /// A fixed vector the verifying side can be tested against without running
    /// the portal. Ed25519 over the canonical string; signature and key are the
    /// raw 64 and 32 bytes, unpadded base64url, so the key drops into a JWK `x`
    /// member and `crypto.verify(null, ...)` checks it with no dependencies.
    #[test]
