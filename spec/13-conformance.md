...
| REQ-51 | P | Both gate scopes are tested at the route-classification level, the always-open set is pinned, and the OB-12 label set carries route *class* rather than path, so no series names a dataset. Whether an `all`-scope deployment refuses each metadata route end-to-end is untested (GAP-33) |
