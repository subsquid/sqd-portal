...
**Mutable doc.** Statuses as of **2026-08-12** (0.13.1; authorization rows re-checked
against baseline `master@e9a1878f863c4d87615bcf21ba1ef79fdf649385` and unchanged). Statuses: **C** covered · **P** partial ·
...
| INV-31 | CT-2 | P | shutdown flip e2e-tested; other conjuncts not; *known-violated* on staleness intent (GAP-2). The spec states that authorization configuration adds no conjunct, which at this baseline is vacuously true |
| INV-35 | CT-8 | U | — |
...
| LIV-5, LIV-6 | CT-2 | U | startup bound unmeasured (S5); no authorization term to measure at this baseline |
...
| INV-38 | CT-10 | C | CT-10 greps the whole portal log and the whole scrape for every secret the stub recorded receiving, after driving admissions, denials and failed exchanges, in both enforcement modes, and checks every response body it reads as it reads it — the egress is the exchange and nothing else. The audit guards itself: it fails if no credential travelled, or if the log and scrape it reads carry no authorization activity. The constant-time conjunct was retired 2026-08-07 rather than met — see the invariant for why the attack it defends against is not reachable through a hashed fingerprint |
