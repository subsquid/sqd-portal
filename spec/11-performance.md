...
| HZ-14 | Usage records are produced at response rate and drained at the sink's rate, so a sink that slows or stops fills P-USAGE-QUEUE and the excess is dropped. The bound is deliberate — the alternative is measurement growing with an outage it cannot fix — but it makes accounting fidelity a function of the sink's availability: the same shape as HZ-7 on the query-log queue, on a separate queue with its own parameters, and with the same drops-not-delays contract (REQ-60, DC-9) | accounting fidelity (REQ-60), PF-1 | CT-11: stop the sink stub and assert responses are unaffected while the reporter keeps trying. Driving traffic past the queue bound is a unit check on the hand-off — the bound is filled, every response is still served whole, and each dropped record is counted — rather than a black-box swarm, which belongs with CT-3 |
| HZ-15 | Usage records carry a route label, a dataset name, an encoding and a key id. Route and encoding come from closed sets, but dataset and key id do not: a Portal serving many datasets to many credentials produces records across their product. The Portal's own metric families carry none of those (drops are labeled by reason alone), so the growth lands on the sink's storage rather than on the scrape — but a record shape that ever put a key id or a client-supplied path on a metric label would reproduce HZ-6 with an attacker-chosen dimension | scrape cost, sink storage (OB-14) | CT-11 / CT-7: assert no usage metric family carries a key id, a dataset, or a request path; series-count audit across a credential-varied churn |
